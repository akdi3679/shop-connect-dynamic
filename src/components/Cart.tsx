...
import { X, ArrowLeft, MapPin, CreditCard, Banknote } from 'lucide-react';
...
const CurrencyIcon = ({ className = "w-24 h-24", color = "inherit" }: { className?: string; color?: string }) => (
  <svg width="2.5em" height="2.5em" viewBox="0 0 100 100" style={{ verticalAlign: 'middle', display: 'inline-block' }} className={className}>
    <text x="70" y="80" fontSize="80" fontFamily="Amiri, serif" direction="rtl"
          fill={color} stroke={color} strokeWidth="1.5" paintOrder="stroke">&#x062F;</text>
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="flex flex-col bg-white/90 backdrop-blur-lg border-l border-white/30 shadow-2xl w-full sm:max-w-sm overflow-hidden">
        ...
        {/* Cart Content */}
        <div className={`flex flex-col h-full transition-transform duration-500 ease-in-out ${currentStep === 'checkout' ? '-translate-x-full' : 'translate-x-0'}`}>
          {cartItems.length > 0 && (
            <Button
              variant="ghost"
              onClick={clearCart}
        ...
        {/* Checkout Content */}
        <div className={`absolute inset-0 flex flex-col h-full transition-transform duration-500 ease-in-out ${currentStep === 'checkout' ? 'translate-x-0' : 'translate-x-full'} bg-white/90 backdrop-blur-lg p-6`}>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-white/30">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span>Subtotal:</span>
                    <div className="flex items-center gap-1">
                      <CurrencyIcon className="w-4 h-4" color="#000000" />
                      <span>{cartTotal.toFixed(2)}</span>
                    </div>
                  </div>
                  <div className="flex justify-between">
          ...
          <Button className="w-full rounded-xl bg-black hover:bg-gray-800 text-white font-semibold py-3 mt-4">
            Place Order
          </Button>
        </div>
      </SheetContent>
