...
import { X, ArrowLeft, CreditCard, Banknote, MapPin, User } from 'lucide-react';
...
const CurrencyIcon = ({ className = "w-24 h-24", color = "#0F0F0F" }: { className?: string; color?: string }) => (
...
const DefaultSignatureIcon = () => (
  <svg viewBox="0 0 1024 1024" className="w-12 h-12 opacity-30" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#6B7280">
        ...
        <>
          ...
          <div className="mt-auto pt-4 border-t border-white/20">
            ...
            <Button 
              ...
              className="w-full mt-4 rounded-xl bg-black hover:bg-gray-800 text-white font-semibold py-3"
            >
              Proceed to Checkout
            </Button>
          </div>
  ...
  const CheckoutContent = () => (
    <div className="flex flex-col h-full">
      <Button
        ...
        className="flex items-center gap-2 mb-4 text-sm font-medium text-black hover:text-gray-600 hover:bg-gray-50 px-2 py-1 h-auto rounded-md w-fit"
      >
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-white/30">
          ...
          <div className="space-y-3">
            <Input
              ...
              className="rounded-lg bg-white/80 border-white/30 text-black"
            />
            <div className="flex items-start gap-2">
              <MapPin className="h-5 w-5 text-gray-500 mt-2.5" />
              <Input
      ...
      <Button 
        className="w-full mt-6 rounded-xl bg-black hover:bg-gray-800 text-white font-semibold py-3"
      ...
      <SheetContent className="flex flex-col bg-white/90 backdrop-blur-lg border-l border-white/30 shadow-2xl w-full sm:max-w-sm">
        <SheetHeader className="pb-4">
          <SheetTitle className="text-xl font-bold text-black">
            {currentStep === 'cart' ? 'Your Cart' : 'Checkout'}
          </SheetTitle>
        </SheetHeader>
        
        <div className={`transition-transform duration-500 flex-1 ${
          currentStep === 'checkout' ? 'transform translate-x-0' : 'transform translate-x-0'
        }`}>
          {currentStep === 'cart' ? <CartContent /> : <CheckoutContent />}
        </div>
      </SheetContent>
    </Sheet>
  );
